...
class ListBag:
    ...
    def grab(self) -> any:
        ...
        """
        if self._elems == []:
            raise ValueError("bag.remove(): grab from empty bag")
        return_val = random.choice(self._elems)
        self._elems.remove(return_val)
        return return_val
    ...
    def __add__(self, other: 'ListBag') -> 'ListBag':
        ...
        """
        if not isinstance(self, ListBag):
    ...
    def __eq__(self, other: 'ListBag') -> bool:
        ...
        return sorted(self._elems) == sorted(other._elems)
